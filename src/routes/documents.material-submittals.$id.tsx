...
export const Route = createFileRoute("/documents/material-submittals/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.id.toUpperCase()} — Material Submittal — SubmitLog` },
    ],
  }),
  loader: ({ params }) => {
    const doc = materialSubmittals.find((d) => d.id === params.id);
...
function DetailPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 py-6">
      {/* Header */}
      <Link to="/documents/material-submittals" className="mb-3 inline-flex items-center gap-1 text-[12px] text-muted-foreground hover:text-foreground">
        <ChevronLeft className="h-3.5 w-3.5" /> Back to Material Submittals
      </Link>
        ...
        <TabsList className="bg-transparent border-b border-border rounded-none w-full justify-start p-0 h-auto">
          {[
            ...
            <TabsTrigger
              key={v}
              value={v}
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-3 h-9 text-[12.5px] gap-1.5"
            >
              ...
              <Section title="Metadata">
                <dl className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
                  {[
                    ...
                    ["Submit date", doc.submitDate],
                    ["Reply date", doc.replyDate ?? "—"],
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/10 via-card to-card p-4">
                ...
                <p className="text-[12.5px] leading-relaxed text-foreground/85">
                  Submittal includes compliance certificates and shop drawings for {doc.title.toLowerCase()}.
                  No discrepancies detected against project specifications. Consider verifying delivery lead times with supplier.
                </p>
                <Button variant="ghost" size="sm" className="mt-3 h-7 gap-1.5 text-[11.5px] text-primary hover:text-primary">
                  Regenerate
                </Button>
              </div>
              ...
              <Section title="Workflow">
                <ol className="relative space-y-3 border-l border-border pl-4">
                  {[
                    { label: "Submitted by contractor", time: doc.submitDate, done: true },
                    { label: "Engineer review", time: "in progress", done: doc.status !== "SUBMITTED", active: doc.status === "SUBMITTED" },
                    { label: "Owner approval", time: "—", done: doc.status === "APPROVED" || doc.status === "APPROVED AS NOTED" },
                  ].map((s, i) => (
                    <li key={i} className="relative">
                      <span className={`absolute -left-[21px] top-0.5 grid h-3.5 w-3.5 place-items-center rounded-full border-2 ${s.done ? "border-success bg-success" : s.active ? "border-primary bg-background" : "border-border bg-background"}`} />
                  ...
                  {["Sarah Chen", "Marco Rossi", "Priya Nair"].map((n) => (
                    <div key={n} className="flex items-center gap-2 rounded-full border border-border bg-background px-2 py-1">
                      <Avatar className="h-5 w-5">
                        <AvatarFallback className="bg-primary/15 text-[9px] font-semibold text-primary">{n.split(" ").map((p) => p[0]).join("")}</AvatarFallback>
                      </Avatar>
